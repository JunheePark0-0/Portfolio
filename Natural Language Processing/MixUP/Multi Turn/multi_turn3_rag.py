...
def call_api_rag_multi_turn(client, model, text, prompt_template, retriever):
    """
    [Turn 1] RAG 검색 결과(Few-shot) + 의미 보존 중심 번역
    [Turn 2] 자연스러운 윤문
    """
    try:
        # -------------------------------------------------
        ...
        for i, doc in enumerate(docs, 1):
            # 메타데이터에 정답('answer')이 있다고 가정 (없으면 doc.page_content만 사용)
...
def main():
    ...
    parser.add_argument("--output", default="submissions/train_rag_multi_turn3.csv", help="Output CSV path")
    parser.add_argument("--model", default="solar-pro2", help="Model name")
    parser.add_argument("--prompt", default="prompts/prompt_8.py", help="Path to prompt file")
